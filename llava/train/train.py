...
class ModelArguments:
    ...
    mm_use_think_token: bool = field(default=False)
...
def read_image_patch(patch_info):
    if 'img_path' in patch_info.keys():
        ...
        if '/url_b64/' in image_file_name or '/url_base64/' in image_file_name or 'tianqincai/WIT' in image_file_name:
            force_base64 = True
        else:
            force_base64 = False
        ...
        with open(image_file_name, 'rb') as f:
            ...
            elif 'image_encoding' in patch_info.keys() and patch_info['image_encoding'] == 'base64':
                image = Image.open(io.BytesIO(base64.b64decode(f.read(file_size).decode()))).convert("RGB")
            else:
                image = Image.open(io.BytesIO(f.read(file_size))).convert("RGB")
    
    if 'force_resize' in patch_info.keys():
        resize_info = patch_info['force_resize']
        if type(resize_info) is str:
            cur_base, cur_ps = resize_info.split('x')
            cur_base = int(cur_base)
            cur_ps = int(cur_ps)
            if cur_ps > 0:
                image = resize_images(image, cur_ps, cur_base)
            else:
                image = image.resize((cur_base, cur_base), Image.BILINEAR)
        else:
            h, w = resize_info
            h = max(h, 64)
            w = max(w, 64)
    ...
    elif 'resize' in patch_info.keys():
